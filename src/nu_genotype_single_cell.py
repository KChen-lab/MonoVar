...
class Single_cell_genotype_records:
	...
	# 	return self.current_cell_ftr_list.Prob_Reads_Given_Genotype_Genotyping(self.j)
	...
	def other_cells_genotype_prob(self, gt_flag, nCr_matrix):
		if self.other_cell_list_len == 0:
			...
			for l in range(0, self.residual_mat.dim[0]):
				...
				prob = prob + coeff * self.residual_mat.denom_prob_matrix[l, self.residual_mat.dim[1]-1] * self.prior_variant_allele[l+gt_flag]
			return prob
